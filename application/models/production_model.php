		...
		if($val->num_rows() == 1){
			...
			if($ctr == '0'){
		    	...
		    	foreach($q->result() as $row)
				{
					...
					$this -> db -> join('purchases', 'purchases.supplier_id = suppliers.supplier_id', 'left');
					$this -> db -> join('purchase_orders', 'purchase_orders.order_reference = purchases.purchase_reference', 'right');
